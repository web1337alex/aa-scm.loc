<?php
global $CoreTheme;
$headerMenu = $CoreTheme::getMenu('headerMenu');
?>
<header class="header-5">
    <div class="header">
        <div class="container">
            <div class="row">
                <div class="col-1 col-xs-2 col-md-1 header__burger">
                    <span class="header__icon"></span>
                </div>
                <div class="col-1 col-xs-2 col-md-1 col-lg-2 header__logo">
                    <a href="<?= get_site_url() ?>" class="header__logoLink">
                        <?= wp_get_attachment_image(get_option('site_icon'), 'full') ?>
                    </a>
                </div>
                <div class="col-2 col-xs-4 col-md-2 col-lg-2 header__title">
                    <a href="<?= get_site_url() ?>" class="header__titleLink"><?= $CoreTheme::$bloginfo['title'] ?></a>
                </div>
                <div class="col-7 col-lg-6 header__menu">
                    <nav class="header__nav">
                        <ul>
                            <? foreach ($headerMenu as $menuItem): ?>
                                <? if ($menuItem->level_menu == 1): ?>
                                    <li><a href="<?= $menuItem->url ?>"><?= $menuItem->title ?></a></li>
                                <? endif ?>
                            <? endforeach ?>
                        </ul>
                    </nav>
                </div>
                <div class="col-2 col-md-8 col-lg-2 header__phone">
                    <div class="header__contacts">
                        <a class="header__tel" href="tel:<?= $CoreTheme::$settings['clean_phone'] ?>"><?= $CoreTheme::$settings['phone'] ?></a>
                        <a class="header__call" href="<?= $args['link']['url'] ?>"><?= $args['link']['text'] ?></a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>